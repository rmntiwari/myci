                ...
                <?php
                    ...
                    foreach($categories as $category){ ?>
                       ...
                       <tr>
                           ...
                           <td><?=$category->cat_keyword?></td>
                           <td>
                               <button class="btn btn-sm btn-success" onclick="window.location.href='<?php echo base_url('catsubcat_controller/editcategory/'.$category->cat_id)   ?>'"><i class="fa fa-edit"></i></button>
                               <button class="btn btn-sm btn-danger" onclick="window.location.href='<?php echo base_url('catsubcat_controller/editcategory/'.$category->cat_id)  ?>'"><i class="fa fa-trash"></i></button>
                           </td>
                       </tr>

                 <?php    }      ?>

               <!--  <?php for($row = 0; $row<count($categories); $row++):?>
               <tr>
                   <td></td>
                   <td><img class="image-border" src="<?=base_url().'uploads/category/'.$categories[$row]->image?>"/></td>
                   <td><?=$categories[$row]->cat_name?></td>
                   <td><?=$categories[$row]->cat_keyword?></td>
                   <td>
